            ...
            <div className="container mx-auto px-4 py-3 flex items-center justify-around">
                <div className="hidden lg:flex items-center gap-2">
                    ...
                    <Button variant="ghost" size="sm" onClick={handleThemeToggle}
                            aria-label={themeToggleButtonAriaLabel}>
                ...
                <div className="lg:hidden w-full flex items-center justify-between gap-2">
                    <section className="flex items-center gap-4"> {/* Added mt-4 for spacing */}
                        {content.socialLinks.map((link) => { // Use dynamic social links
                            const IconComponent = link.icon; // Get the icon component from content
                            return (
                                <Link
                                    className="p-1 bg-primary text-primary-foreground rounded-full shadow-md hover:bg-accent hover:text-accent-foreground transition-colors duration-200"
                                    key={link.name} href={link.href} target="_blank" rel="noopener noreferrer">
                                    <IconComponent
                                        className="h-[1rem] w-[1rem] text-primary-foreground hover:text-accent transition-colors"
                    ...
                    <section>
                        ...
                        >
                            ...
                            <span
                                className="sr-only">{isLoadingTranslation ? content.translationButton.loading : translationButtonText}</span>
                        </Button>
